...
def main():
    ...
    global multiple
    global mode
    mode = str(input("Selecione o modo: GET ou HEAD: "))
    if mode == "":
        mode = "get"
    else:
        mode = str(mode)
    ...
    if ip == "":
        ...
        return main()
    page = str(input("PAGINA DE ATAQUE: (dedault: /) "))
    if page == "":
        page = "/"
    ...
    if th_num == "":
        ...
        th_num = int(th_num)
    #if mode == "get" or mode == "GET":
    N = str(input("Desteja baixar a lista de proxys? ?(y/n): "))
    if N == "n" or N =="N":
        pass
    else:
        clone()
    out_file = str(input("Digite: socks5.txt: "))
    if out_file == "":
        out_file = "socks5.txt"
    else:
    ...
    time.sleep(0.3)
    ans = str(input("Deseja checkar a lista de proxys? (y/n, USE=y): "))
    if ans == "n" or ans =="N":
        pass
    else:
        ms = str(input("Escreva o número 2: "))
        if ms == "":
            ms = int(1)
        else :
            ms = int(ms)
    check_socks()
    proxies = open(out_file).readlines()
    multiple = str(input("Escreva o número 100: "))
    if multiple == "":
        multiple = int(100)
    else:
